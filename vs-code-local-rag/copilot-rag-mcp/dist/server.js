...
function dedupe(hits) {
    const kept = [];
    for (const h of hits) {
        const overlap = kept.find((k) => k.payload.path === h.payload.path &&
...
const server = new McpServer({ name: "copilot-rag-mcp", version: "0.1.0" }, {
    instructions: "Local semantic index of the user's codebase. Prefer get_context for grounding " +
...
server.registerTool("search_code", {
    title: "Search code",
    description: "Semantic search over the indexed codebase. Returns the top matching code " +
...
server.registerTool("get_context", {
    ...
    description: "Returns the most relevant code for a task, packed to stay under a token " +
    ...
    for (const h of hits) {
        ...
        if (used + cost > budget)
            continue; // skip oversized; keep filling with smaller ones
        blocks.push(block);
        used += cost;
    }
    if (blocks.length === 0) {
...
server.registerTool("list_indexed_files", {
...
server.registerTool("reindex", {
    ...
    description: "Incrementally re-embed changed files into the vector store. Pass force=true " +
    ...
    return {
        content: [
            {
                type: "text",
                text: `Reindex complete. embedded=${r.indexedFiles} unchanged=${r.skippedFiles} ` +
...
async function main() {
    ...
    process.stderr.write(`copilot-rag-mcp ready. repo=${config.repoRoot} collection=${config.collection}\n`);
}
main().catch((err) => {
