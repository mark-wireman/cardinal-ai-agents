...
import { VectorStore, type ChunkPayload } from "./vectorstore.js";
...
interface Manifest {
  model: string;
  files: Record<string, string>; // repo-relative path -> fileHash
}
...
function buildIgnore(): Ignore {
  const ig = (ignore as unknown as () => Ignore)().add(DEFAULT_IGNORES);
...
export async function indexRepo(force = false): Promise<{
  indexedFiles: number;
  skippedFiles: number;
  removedFiles: number;
  totalChunks: number;
  ...
  if (prev.model !== config.embedModel) force = true;

  const next: Manifest = { model: config.embedModel, files: {} };
  ...
  for (const rel of files) {
    ...
    const texts = rawChunks.map(
      (c) => `// file: ${rel} (lines ${c.startLine}-${c.endLine})\n${c.text}`,
    );
    ...
    indexedFiles += 1;
    if (indexedFiles % 25 === 0) log(`  ...${indexedFiles} files embedded`);
  }
  ...
  log(
    `Done. embedded=${indexedFiles} unchanged=${skippedFiles} removed=${removedFiles} chunks=${totalChunks}`,
  );
